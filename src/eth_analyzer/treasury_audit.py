...
TRANSFER_TOPIC = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628a6f9b5a4df523b3ef"
...
def _config(value: str) -> dict[str, Any]:
    try:
        r=value[2:]
        return {"plugin":_addr(r[0:64]),"userOpValidationFunction":{"plugin":_addr(r[64:128]),"functionId":int(r[128:192],16)},"runtimeValidationFunction":{"plugin":_addr(r[192:256]),"functionId":int(r[256:320],16)}}
...
def _identity(eth: EthRpc, address: str, block: str) -> dict[str,Any]:
    ...
    for name,target in TARGETS.items():
        ...
        configs[name]=_safe(eth,address,CONFIG+arg,block)
        if "raw" in configs[name]: configs[name]["decoded"]=_config(configs[name]["raw"])
        hooks[name]=_safe(eth,address,HOOKS+arg,block)
        if "raw" in hooks[name]: hooks[name]["decoded"]=_exec_hooks(hooks[name]["raw"])
        pre[name]=_safe(eth,address,PRE_HOOKS+arg,block)
    ...
    out["upgradeAuthorization"]={"status":"unproven","upgradeSelectors":{"upgradeTo":TARGETS["upgradeTo"],"upgradeToAndCall":TARGETS["upgradeToAndCall"]},"note":"UUPS selectors expose an upgrade surface; authorization must be established from source or a successful historical authorization path."}
    return out
...
def _transfers(eth: EthRpc, address: str, start: int, end: int) -> list[dict[str,Any]]:
    ...
    for topic in ([TRANSFER_TOPIC,padded,None],[TRANSFER_TOPIC,None,padded]):
        for log in _logs(eth,{"topics":topic},start,end):
            t=log.get("topics",[])
            if len(t)<3: continue
...
def _implementation_history(eth: EthRpc, address: str, start: int, end: int) -> list[dict[str,Any]]:
    # Detect implementation changes by reading the EIP-1967 slot at every proxy-log block.
    logs=_logs(eth,{"address":address},start,end); blocks=sorted({_n(x["blockNumber"]) for x in logs}); blocks=[start]+blocks+[end]; blocks=sorted(set(blocks)); history=[]; previous=None
    for block in blocks:
        impl=_addr(eth.get_storage_at(address,IMPLEMENTATION_SLOT,_tag(block)).result)
        if impl.lower()!=str(previous).lower():
            history.append({"blockNumber":_tag(block),"implementation":impl,"evidence":"historical EIP-1967 storage read"}); previous=impl
    return history

def _classify(eth: EthRpc, tx_hash: str, treasury: str) -> dict[str,Any]:
    tx=eth.get_transaction_by_hash(tx_hash).result or {}; receipt=eth.get_transaction_receipt(tx_hash).result or {}; to=(tx.get("to") or "").lower(); sel=_sel(tx.get("input"))
    if to==treasury.lower(): path="direct_treasury_call"
    elif to==ENTRY_POINT and sel==HANDLE_OPS: path="erc4337_handleOps"
    else: path="indirect_or_other"
...
def collect_treasury_audit(eth: EthRpc, address: str, from_block: str, to_block: str) -> TreasuryAudit:
    start=_n(from_block); end=_n(to_block)
    if end<start: raise ValueError("--to-block must be >= --from-block")
    final=_tag(end); identity=_identity(eth,address,final); transfers=_transfers(eth,address,start,end); hashes=sorted({x["transactionHash"] for x in transfers if x.get("transactionHash")}); txs=[_classify(eth,h,address) for h in hashes]
    inbound=[x for x in transfers if x["direction"]=="inbound"]; outbound=[x for x in transfers if x["direction"]=="outbound"]
    report={"schemaVersion":"1.0","scope":{"address":address,"fromBlock":from_block,"toBlock":to_block,"readOnly":True},"treasuryIdentity":{**identity,"implementationHistory":_implementation_history(eth,address,start,end)},"authority":{"nativeOwner":identity.get("getNativeOwner"),"installedPlugins":identity.get("getInstalledPlugins"),"entryPoint":identity.get("getEntryPoint"),"pluginManager":identity.get("PLUGIN_MANAGER"),"nativeExecutionPermissions":identity.get("executionFunctionConfigs"),"userOpValidation":identity.get("executionFunctionConfigs",{}).get("validateUserOp"),"runtimeValidation":identity.get("executionFunctionConfigs"),"hooks":{"execution":identity.get("executionHooks"),"preValidation":identity.get("preValidationHooks")},"upgradeAuthorization":identity.get("upgradeAuthorization")},"assetHistory":{"inbound":inbound,"outbound":outbound,"tokenContracts":sorted({x["token"] for x in transfers}),"recipients":sorted({x["to"] for x in outbound}),"senders":sorted({x["from"] for x in inbound}),"transferCount":len(transfers)},"transactionPaths":{"transactions":txs,"counts":{"directCalls":sum(x["path"]=="direct_treasury_call" for x in txs),"erc4337UserOps":sum(x["path"]=="erc4337_handleOps" for x in txs),"indirectOrOther":sum(x["path"]=="indirect_or_other" for x in txs)},"selectors":TARGETS},"applicationCorrelation":{"status":"incident-correlation-record","pod":None,"contribution":{"knownTransaction":"0xc2224beade1fe849d295b7588d3d0e46d71de4c52c97a3ef750bcd4424336001","source":"0x68af0efa38a801821116dfa95642eddafd07df35","amount":"4"},"stateUpdate":{"contract":"0x100012Cd3BFBd6af56dAd2E0a874001f1De5038e","operation":"upsert","knownRevertSelector":"0x326d141f"},"failures":[{"type":"application_state_write","selector":"0x326d141f","status":"observed","rootCause":"not established"}]},"evidence":{"sourceMethod":"historical eth_getStorageAt/eth_call + eth_getLogs + transaction/receipt reads","limitations":["Upgrade authorization is intentionally not inferred from selector presence.","Deep ERC-4337 UserOperation decoding remains in transaction analysis; this audit classifies EntryPoint handleOps paths.","Application correlation is recorded from the known incident and is not a live UnitPay API query."]}}
    return TreasuryAudit(address,address,from_block,to_block,report)
