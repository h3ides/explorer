...
def _decode_inner_call(data: str) -> dict[str, Any]:
    ...
    raw = data[10:]
    result: dict[str, Any] = {"selector": selector, "method": next((name for name, value in KNOWN_SELECTORS.items() if value == selector), None), "raw": data}
    try:
        if selector == KNOWN_SELECTORS["transfer"]:
            ...
            result["arguments"] = {"from": _address_word(_word(raw, 0)), "to": _address_word(_word(raw, 1)), "amount": _uint_word(_word(raw, 2))}
        elif selector == KNOWN_SELECTORS["execute"]:
            target = _address_word(_word(raw, 0)); value = _uint_word(_word(raw, 1)); data_offset = _uint_word(_word(raw, 2)); inner = _bytes_at(raw, data_offset)
            result["arguments"] = {"target": target, "value": value, "data": _decode_inner_call(inner)}
        elif selector == KNOWN_SELECTORS["executeFromPlugin"]:
            data_offset = _uint_word(_word(raw, 0)); result["arguments"] = {"data": _decode_inner_call(_bytes_at(raw, data_offset))}
        elif selector == KNOWN_SELECTORS["executeFromPluginExternal"]:
            target = _address_word(_word(raw, 0)); value = _uint_word(_word(raw, 1)); data_offset = _uint_word(_word(raw, 2)); inner = _bytes_at(raw, data_offset)
            result["arguments"] = {"target": target, "value": value, "data": _decode_inner_call(inner)}
        elif selector == KNOWN_SELECTORS["executeBatch"]:
            offset = _uint_word(_word(raw, 0)); base = offset * 2; length = int(raw[base:base + 64], 16); calls=[]; head_start=base + 64
            for i in range(length):
                tuple_offset=int(raw[head_start+i*64:head_start+(i+1)*64],16); item=head_start+tuple_offset*2; target=_address_word(raw[item:item+64]); value=_uint_word(raw[item+64:item+128]); data_rel=int(raw[item+128:item+192],16); data_start=item+data_rel*2; data_len=int(raw[data_start:data_start+64],16); inner="0x"+raw[data_start+64:data_start+64+data_len*2]
...
def _decode_logs(logs: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for log in logs or []:
        ...
        entry={"address":log.get("address"),"topics":topics,"data":log.get("data"),"logIndex":log.get("logIndex")}
        if topics and topics[0].lower()==TRANSFER_TOPIC:
            entry["event"]="Transfer"
            if len(topics)>=3:
                entry["from"]=_address_word(topics[1][2:]); entry["to"]=_address_word(topics[2][2:])
            if isinstance(log.get("data"),str) and log["data"].startswith("0x"):
                try: entry["amount"]=int(log["data"],16)
...
def collect_transaction(eth: EthRpc, tx_hash: str) -> TransactionSnapshot:
    tx=eth.get_transaction_by_hash(tx_hash).result; receipt=eth.get_transaction_receipt(tx_hash).result; decoded={}
    if tx: decoded["topLevel"]=_decode_inner_call(tx.get("input","0x"))
    decoded["logs"]=_decode_logs(receipt.get("logs") if receipt else None)
    if receipt:
        decoded["status"]=receipt.get("status"); decoded["gasUsed"]=receipt.get("gasUsed"); decoded["blockNumber"]=receipt.get("blockNumber")
...
def write_transaction(snapshot: TransactionSnapshot,path: str)->None:
    with open(path,"w",encoding="utf-8") as fh:
        json.dump({"txHash":snapshot.tx_hash,"transaction":snapshot.transaction,"receipt":snapshot.receipt,"decoded":snapshot.decoded},fh,indent=2); fh.write("\n")
